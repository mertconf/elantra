...
module.exports = {
  ...
  async execute(client, interaction, data, args) {
    ...
    //ElantraCoin
    let NC = new Intl.NumberFormat().format(userData.ElantraCoin);
    if (userData.ElantraCoin)
      embed.fields.push(
        {
          name: '**»** ElantraCoin',
          value: `**•** \`${NC} ElantraCoin\` <:ElantraCoin:946801199976419339>`,
        }
      );

    //Premium
    let Premium = userData.ElantraPremium
      ? userData.ElantraPremium > Date.now()
        ? `${humanize(userData.ElantraPremium - Date.now(), { language: "tr", round: true, largest: 2 })}`
