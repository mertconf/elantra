  ...
  async execute(client, interaction, data) {
    ...
    if (getCommand == "bilgi") {
      ...
      const webhook = await getChannel.createWebhook({
        name: 'Elantra Logger',
        avatar: 'https://media.discordapp.net/attachments/727501328519004200/910108789796110346/Elantra-Test-Logo-Kare.png',
        reason: `Elantra Log Sistemi • ${interaction.user.tag} tarafından açıldı.`
      }).catch(err => {
        client.logger.error(err);
        return interaction.editReply(`Log kanalını ayarlayamadım. Yetkilerimle ilgili bir sorun olabilir. Çözemezsen destek sunucumuzda bildirebilirsin.\n\nhttps://discord.gg/MXhDCCatvy`);
      });
