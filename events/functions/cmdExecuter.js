...
module.exports = async (client, interaction, cmd, guildData, userData, args = null) => {
  ...
  try {
    ...
    const data = {
      ...
      premium: (userData.ElantraPremium && userData.ElantraPremium > Date.now()),
      guildIsBoosted: (guildData.ElantraBoost?.users?.length ? true : false)
    };
        ...
        return interaction.reply({
          embeds: [{
            ...
            description:
              ...
              `**•** Ya da en iyisi Elantra Premium abonesi olabilirsin. \`/premium Bilgi\``,
          }],
          components: [
            {
              type: 1, components: [
        ...
        embeds: [{
          ...
          description:
            data.premium ?
              ...
              :
              `**»** Bu komutu tekrar kullanabilmen için **${Math.ceil((userDataCache_lastCmds_thisCmd + cmdCooldown - Date.now()) / 1000)} saniye** beklemelisin.\n` +
              `**•** Elantra Premium ile bekleme sürelerini kısaltabilirsin. \`/premium Bilgi\``
        }],
        ephemeral: true
      });
    }
